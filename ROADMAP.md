...
2. **Backend freedom production wiring.** Provider abstractions and pure routing
   ...
   seams. `fast.research_deepening` and `fast.analysis_workbook` now also
   resolve through the capability router (reasoning role) with body-free route
   ...
   services during normal runs. All five routes record body-free usage
   ...
   research deepening skips gap analysis and the workbook falls back to
   collected insights with the same fail-closed route record. The next
