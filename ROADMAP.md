...
- Semantic local stage evidence shipped in `core/local_stage_eval.py` and `core/cli_local_stage_eval.py`: `--eval-local-stage website-summary --eval-local-stage-semantic-judge` runs a local OpenAI-compatible semantic judge pass, writes body-free semantic eval artifacts, and can feed same-command scorecards as review-only evidence. This is not a promotion gate by itself; broader calibrated samples and agreement checks are still required.
