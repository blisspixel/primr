...
next and why"; if prose below conflicts with that brief, update or retire the
stale roadmap prose rather than creating a second current queue.
...
|------|--------|----------------|
| **v1.39.x** (current) | Operator polish, security floors, routing honesty, maintainable stage ownership | Ship safe defaults and truthful CLI/MCP surfaces while quality instrumentation matures. The next executable candidate is v1.39.17. |
...
| **v1.41** | Backend freedom: measured host promotion + OpenRouter gateway + residual dual-provider cleanup | Unlocks honest OpenAI-only / Anthropic-only / OpenRouter / host / local profiles; depends on 1.40 instruments to judge backends. |
...
**Next implementation candidate:** v1.39.17 removes the deferred
`fast_run_sections` ownership back edge with its original graph-reduction
criteria. v1.39.16 adds Gemini 3.8 Flash model support, expanded OpenRouter
routing catalog and role overrides, and verified multi-provider bakeoff evaluation.
...
3. **Agent control-plane consumption resources and A2A parity.** MCP already has
   ...
   approval token by default before creating a job,
...
- Eight providers wired: xAI (Grok), Google (Gemini), OpenAI, Anthropic, OpenRouter (explicitly enabled preview), Ollama (local), Microsoft Foundry (Azure OpenAI-compatible `/openai/v1/`), Amazon Bedrock (boto3 `converse`)
- Provider abstraction at `src/primr/ai/providers/` - `Provider` ABC, `OpenAICompatibleProvider` (xAI/OpenAI/OpenRouter/Ollama/vLLM), `GeminiProvider`, `AnthropicProvider`, `AzureFoundryProvider`, `BedrockProvider`
- `pick_model_for_role` chooses the best model from configured providers; `primr doctor` shows what each key unlocks; `primr keys test` does live auth-only validation through each provider's free metadata endpoint, with no model generation or token spend, via `Provider.validate_credentials()`
...
- **Planned, not urgent: broad model coverage + estimate-vs-actual evals.** The
  ...
  catalog and validating each is deliberately deferred. Grok 4.6 and OpenAI
  GPT-5.6 are now registered as *available-but-never-default* candidates. Future
  work: (1) register only still-relevant deployment-surface models (Foundry Phi-4;
  Bedrock Nova 2, Gemma 4, DeepSeek R1/V3, Llama 4) and remaining flagships such
  as Claude Fable 5 after verifying each id and price against official docs;
  (2) do **not** eval every model now; when models are evaluated, extend
...
- Output improvement: `primr improve` is deterministic and model-free by
  default. The optional `--improve-agentic` pass and `primr refine` are
  model-backed, expose bounded dry-run quotes, require explicit approval, and
  reserve quoted stage costs against `--budget`. JSON estimate, refusal, and
  result surfaces emit one versioned object.
...
## Backlog and Implementation Ledger

This ledger preserves feature rationale, partial milestones, and completed
slices. It is not ordered at next-PR granularity and intentionally retains some
`DONE` entries for context. Use `docs/NEXT_STEPS.md` for current execution
order and the changelog for release history.
...
- Long-context and cache-token estimate modeling - DONE: `ModelConfig` now
  carries long-context tier metadata for documented OpenAI 1.05M-context models
  (>272K input: 2x input, 1.5x output), `PrimrModels.calculate_cost_breakdown()`
...
- Server-issued approval tokens for the MCP cost-cap-governed execution tools
  ...
  tool, approval-shape hash, approved max cost, and expiry. Execution requires
  both the cap and a matching token by default on every MCP transport.
...
- **Phase A (1.x)**: refresh the model registry to the current generations
  (OpenAI GPT-5.6 / GPT-5.5 / GPT-5.4 families, Anthropic claude-fable-5 / opus-4-8 /
...
- **Supported-Python window**: `requires-python = ">=3.12"`, EOL-driven - 3.10 reaches EOL Oct 2026 and 3.11 Oct 2027, while 3.12 carries security support to Oct 2028. CI runs a `3.12 / 3.13 / 3.14` **hard** supported matrix. Python 3.15.0rc2 was released on September 1, 2026, with final scheduled for October 1; a separate hard Linux preview lane exercises 3.15 without adding a stable classifier. Windows 3.15 remains upstream-blocked because the locked MCP and DOCX stack needs a `pywin32` wheel that does not yet exist for `cp315`. The `.python-version` file pins the dev default. Free-threaded 3.14t is not the default runtime or a release gate. Many locked native dependencies now publish `cp314t` artifacts, but Primr has not validated the complete supported platform matrix or demonstrated an end-to-end benefit. Maintain a nonblocking compatibility and benchmark lane when practical; adoption requires a shared-state audit, confirmation that required extensions do not silently restore the GIL, and a material measured throughput or CPU-stage gain.
...
- **Complexity budget**: add `C901` + `max-complexity` once the documented monsters are refactored (`perform_fast_research` ~1900 lines, `_execute_consulting_research` ~270 lines - backlog item #23).
...
Azure is the validated cloud. See `docs/NEXT_STEPS.md` for any current
hardening work. GCP and AWS templates exist as reference implementations but
are not validated end-to-end.
...
## Condensed Release Ledger

This historical summary is retained for convenience. Canonical release details
live in [the changelog](docs/CHANGELOG.md) and
[GitHub releases](https://github.com/blisspixel/primr/releases).
