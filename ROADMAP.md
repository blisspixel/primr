...
- **`--budget $N` flag — DONE.** Per-run cost ceiling for standard research, activating the existing `CostGuardHook` accounting via `primr.utils.run_budget`. Pre-flight: refuses to start when the dry-run estimate exceeds the ceiling. Mid-run: budget checkpoints at the optional spend stages (Phase 2 research deepening, Phase 5 cross-validation enrichment and contradiction resolution, and Phase 6 strategy generation) sync actual session spend and skip that stage once the ceiling is reached, so the report still ships within budget. Budget is cleared in a `finally` so it can't leak across runs.
