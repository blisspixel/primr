...
3. **Agent control-plane consumption resources and A2A parity.** MCP already has
   ...
   first two resource slices shipped:
   ...
   score/status/count metadata, both without report body content.
...
Planned:
- Expand job-scoped resources for artifact consumption (source appendix, trace summary, usage/cost summary, verification summary, calibration summary) so clients do not need large report bodies in context by default. The artifact metadata and QA summary slices are shipped.
