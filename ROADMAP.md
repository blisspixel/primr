...
- **Output / egress guardrails** - SHIPPED baseline, hardening continuing. `is_safe_url` guards every egress helper; `data/safe_http.py` validates every redirect hop before connecting for `fallback_sources._http_get`, `hiring_signals._http_get`, and Wayback CDX/replay fetches; `data/scraping/net.py` validates every redirect hop manually for sitemap and URL-existence checks; and `HTTPClient.get/head` does the same while preserving pooled sessions. Legacy scraping-tier clients retain final-url redirect validation until they migrate to the same seam. The "no fetch bypasses the SSRF guard" invariant is locked by `tests/security/test_egress_guardrails.py`; intermediate-redirect migration remains tracked for the older scraping clients.
