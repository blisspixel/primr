...
**Status (as of v1.30.0):** most of the 1.x engineering backlog is closed —
...
- **#23 orchestrator refactor** — `perform_fast_research` (~1,900 lines) split
  ...
  the complexity budget. EXTRACTION COMPLETE: all ten stages of
  `perform_fast_research` are out (Batches A–G), each as a module with a
  hermetic test suite; the orchestrator is now a ~295-line coordinator.
  Remaining within #23: `FastRunContext`, the research_agent per-module
  coverage target, the C901 complexity budget, and the
  `_execute_consulting_research` split — per
...
- **Interoperability** — primr presented as a *role* other agents assign work
  to: per-tool capability-scoped authorization (T8), server-issued approval
  tokens, a structured invocation audit log, job-scoped artifact resources,
...
- **Coverage baseline + non-regression ratchet**: measured global branch coverage is **82%** (9,138 tests, branch mode); CI gates at `--cov-branch --cov-fail-under=81` (1-point margin for platform variance). Ratcheted up from 77 after a focused test push: the coverage job now installs the `a2a` extra so the a2a package is counted (its 165 tests previously ran only in the separate uncovered job), plus ~630 new unit tests across `research_orchestrator`, `utils.security`, `skill_pack.evidence`, `data.scrape`, `model_eval`, `mcp_server.skill_pack_tools`, the `agentic` modules, `hiring_signals`, the `scraping` helpers, `ai_strategy`, and `mcp_server.server`. A ratchet that only ever rises — not the brief's blanket 95%, which is the wrong target for heavy I/O / LLM glue. The largest remaining gap is `research_agent.py` (~29%), gated on the Active Queue #23 refactor that makes its monster functions unit-testable rather than mock-padding them now.
...
- **OpenSSF Scorecard + badge** — distinct from the tracked OpenSSF *Secure Coding Guide* audit (a manual code pass): Scorecard is the automated repo-health scorer. Add the `ossf/scorecard-action` on a schedule, publish to the Security tab, and surface the badge in the README. primr should already score well (pinned actions, OIDC trusted publishing, SLSA provenance, the `main-safety` ruleset, hard-gated secret scanning) — Scorecard makes that *externally legible* and ratchets it. The recognized objective bar for "is this repo well-run."
- **CodeQL SAST** as a second, *semantic* security gate beside bandit/Trivy/pip-audit. Bandit is pattern-based and Python-only; CodeQL adds dataflow/taint analysis (free for public repos) into the same Security tab. Low-risk; warn-then-hard once the baseline is clean (the same promotion pattern used for Trivy).
...
- **Published documentation site** (mkdocs-material on GitHub Pages, versioned). The 28 hand-maintained `docs/*.md` files are strong content but aren't navigable or searchable as a site; a built site with search and an auto-generated API reference (`mkdocstrings`, off the `py.typed` signatures) is the last reference-grade gap. Larger effort — sequence after the cheap wins; `.readthedocs.yaml` vs Pages decided at pickup.
