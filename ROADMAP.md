...
3. **Agent control-plane consumption resources and A2A parity.** MCP already has
   ...
   first four resource slices shipped:
   ...
   source appendix metadata, all without report body content.
...
5. **Control plane** (T8 + #21) (2.0): per-tool authz is now shipped for MCP
   ...
   MCP invocation audit is now shipped for tool calls, and the first four
   ...
   summaries. A2A parity and richer trace resources remain next.
...
Planned:
- Expand job-scoped resources for artifact consumption (trace summary, verification summary, calibration summary) so clients do not need large report bodies in context by default. The artifact metadata, QA summary, usage/cost summary, and source appendix summary slices are shipped.
