...
- **`--budget $N` flag - DONE for pre-flight, runtime checkpoints shipped for fast full-report runs.** Per-run cost ceiling for standard research, activating the existing `CostGuardHook` accounting via `primr.utils.run_budget`. Pre-flight: refuses to start when the dry-run estimate exceeds the ceiling. Mid-run fast-path checkpoints at the optional spend stages (Phase 2 research deepening, Phase 5 cross-validation enrichment and contradiction resolution, and Phase 6 strategy generation) sync actual session spend and skip that stage (and, in a multi-strategy run, any remaining strategy documents) once the ceiling is reached. Premium, deep, scrape, and non-fast structured paths are currently estimate-gated only; the CLI dry-run JSON and MCP `estimate_run` payload now state that explicitly. CLI estimates also clamp enabled AI-strategy runs to at least one vendor, matching MCP estimate behavior for empty internal platform tuples. Budget is cleared in a `finally` so it can't leak across runs.
