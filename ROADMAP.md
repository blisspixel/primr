...
- **Model landscape refresh (May 30, 2026 audit)**: `claude-opus-4-8` and `gemini-3.5-flash` are registered in `config/models.py`. Eval-gated follow-up: run the eval harness to decide whether to repoint the **PRO tier** from `gemini-3.1-pro-preview` ($2/$12) to `gemini-3.5-flash` ($1.50/$9 — cheaper *and* benchmarks above 3.1 Pro). It is **not** a writing-tier swap (dearer than `gemini-3.1-flash-lite`). Register `gemini-3.5-pro` (June) and `gemini-omni` once their API slugs go GA. Re-audit before each major eval per "Model Adaptability".
...
Already shipped: SSRF guard on every outbound URL (`primr.utils.security`), content sanitization for prompt-injection defense, a hard-gated hardcoded-secret scanner, MCP JWT auth, A2A loopback/auth, `CostGuardHook` budget caps, and the supply-chain gates above. Tracked next (scoped to the real surface):

- **Indirect prompt-injection hardening**: primr's highest-leverage AI-sec risk — malicious instructions embedded in scraped pages / job postings / sitemaps hijacking the LLM. Formalize "external text is *data*, never *instructions*" (consistent delimiting/spotlighting of scraped content in prompts) and back it with an injection red-team corpus in the eval harness so regressions are measurable. Builds on the existing content sanitization.
- **Output / egress guardrails**: an injected instruction must not be able to widen the agent's own tool/URL scope or exfiltrate. URL fetches are already SSRF-checked; make "the LLM cannot expand its own capabilities" an explicit, tested invariant (a natural `deal`/guardrail contract).
- **Agentic trust boundaries (MCP/A2A)**: capability-scoped tokens + per-tool authorization so every tool invocation is authenticated, capability-bounded, and cost-bounded. Folds into Active Queue #11 (constrained agent permissions) and #21 (agent control-plane hardening).
- **Secret / log redaction**: a `SecretMaskingFilter` is now wired into both the console and file logging handlers (`utils/logging_config.py`), so every record is run through `mask_sensitive_data` at the sink regardless of caller discipline — defense-in-depth, not opt-in. Provider key patterns include xAI (primr's primary), Google, OpenAI, Anthropic, GitHub, AWS, Slack, and JWT. Remaining: extend the same guarantee to non-log artifacts (usage records, run-state JSON) and add a property test asserting no key-shaped string reaches any persisted artifact.
- **Scoped threat model + adversarial eval**: a short MITRE-ATLAS-style threat model covering only the client/scraper/agent surface, plus an injection/jailbreak scorecard slice in the eval harness. Document residual-risk acceptance.
