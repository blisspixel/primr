...
3. **Agent control-plane consumption resources and A2A parity.** MCP already has
   ...
   first three resource slices shipped:
   ...
   approval, execution, and artifact-count metadata, all without report body
   content.
...
5. **Control plane** (T8 + #21) (2.0): per-tool authz is now shipped for MCP
   ...
   MCP invocation audit is now shipped for tool calls, and the first two
   job-scoped resource slices now cover artifact metadata and compact QA
   summaries. A2A parity and richer source/trace/usage resources remain next.
...
Planned:
- Expand job-scoped resources for artifact consumption (source appendix, trace summary, verification summary, calibration summary) so clients do not need large report bodies in context by default. The artifact metadata, QA summary, and usage/cost summary slices are shipped.
