...
_BODY = (
    "## What This Skill Does\n\nConduct a SAM assessment via the SAM+ Hub.\n\n"
...
def _pack() -> SkillPack:
    role = Role(
        name="sam-plus-consultant",
        display_name="SAM+ Consultant",
        confidence="Inferred",
        summary="Test role.",
    ...
    "evals": [
        {
            ...
            "expected_output": "A ranked table of license waste with SAM+ Hub as source.",
            "assertions": [
                "names the SAM+ Hub as the data source",
...
def test_benchmark_skill_with_beats_baseline():
    ...
    def _mock(prompt: str, **kwargs: Any) -> str:
        ...
        if "ASSERTIONS" in prompt and "OUTPUT TO GRADE" in prompt:
            ...
            # mentions the SAM+ Hub, so key on the output marker instead): the
        ...
        # the SAM+ Hub) in the system prompt; baseline does not.
        if "SAM+ Hub" in system:
            return "Here is a ranked table sourced from the SAM+ Hub."
