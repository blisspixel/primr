...
def test_citations_all_sources_dedupe():
    ...
    cites = p.extract_citations_from_content(content, all_sources_sections=True, dedupe_urls=True)
    assert len(cites) == 1
...
def test_citations_inline_placeholder_fallback():
    ...
    cites = p.extract_citations_from_content(content, include_inline_placeholders=True)
...
def test_extract_interaction_citations_uses_inline():
    interaction = SimpleNamespace(outputs=[SimpleNamespace(text="Body [cite: 5].")])
...
def test_search_queries_count_output_level():
    ...
    interaction = SimpleNamespace(outputs=[SimpleNamespace(grounding_metadata=meta)])
