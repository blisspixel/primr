...
error_message_strategy = st.text(
    alphabet=st.characters(whitelist_categories=("L", "N", "P", "S", "Z")), min_size=1, max_size=100
...
def create_rate_limit_error(
    message: str, retry_after_seconds: float, context: dict[str, Any] | None = None
) -> TypedRateLimitError:
    """Create a TypedRateLimitError for testing."""
    return TypedRateLimitError(
        message=message, retry_after_seconds=retry_after_seconds, context=context or {}
    )


def create_quota_error(
    message: str, hours_until_reset: float, context: dict[str, Any] | None = None
) -> QuotaError:
    """Create a QuotaError for testing."""
    reset_time = datetime.now() + timedelta(hours=max(0.01, hours_until_reset))
    return QuotaError(message=message, quota_reset_time=reset_time, context=context or {})


def create_network_error(
    message: str, host: str = "example.com", port: int = 443, context: dict[str, Any] | None = None
) -> TypedNetworkError:
    """Create a TypedNetworkError for testing."""
    return TypedNetworkError(message=message, host=host, port=port, context=context or {})
...
def create_validation_error(
    ...
    return PrimrValidationError(
        message=message, field_errors=field_errors or {}, context=context or {}
    )


def create_auth_error(
    message: str, auth_method: str = "api_key", context: dict[str, Any] | None = None
) -> AuthenticationError:
    """Create an AuthenticationError for testing."""
    return AuthenticationError(message=message, auth_method=auth_method, context=context or {})
...
class TestRetryEligibilityByErrorType:
    ...
    **Validates: Requirements 2.1, 2.2, 2.3**
    ...
    @given(message=error_message_strategy, policy=retry_policy_strategy, attempt=attempt_strategy)
    ...
    def test_transient_error_is_retryable_when_under_max_retries(
        ...
        expected = attempt < policy.max_retries and error.category in policy.retryable_categories
    ...
    @given(message=error_message_strategy, policy=retry_policy_strategy, attempt=attempt_strategy)
    @settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow])
    def test_auth_error_is_never_retryable(self, message: str, policy: RetryPolicy, attempt: int):
    ...
    @given(message=error_message_strategy, policy=retry_policy_strategy)
    @settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow])
    def test_non_primr_error_is_never_retryable(self, message: str, policy: RetryPolicy):
    ...
    @given(message=error_message_strategy, policy=retry_policy_strategy)
    @settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow])
    def test_at_max_retries_is_not_retryable(self, message: str, policy: RetryPolicy):
    ...
    @given(message=error_message_strategy, policy=retry_policy_strategy)
    @settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow])
    def test_above_max_retries_is_not_retryable(self, message: str, policy: RetryPolicy):
...
class TestDelayCalculationCorrectness:
    ...
    **Validates: Requirements 2.4, 2.5**
    ...
    @given(message=error_message_strategy, policy=retry_policy_strategy, attempt=attempt_strategy)
    @settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow])
    def test_delay_is_always_non_negative(self, message: str, policy: RetryPolicy, attempt: int):
    ...
    @settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow])
    def test_delay_respects_max_delay(self, message: str, policy: RetryPolicy, attempt: int):
    ...
    @given(message=error_message_strategy, policy=retry_policy_strategy)
    @settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow])
    def test_delay_increases_with_attempt(self, message: str, policy: RetryPolicy):
    ...
    def test_quota_error_past_reset_time_returns_zero(self, message: str, policy: RetryPolicy):
...
class TestRetryHistoryAttachment:
    ...
    **Validates: Requirements 2.8**
    ...
    @given(message=error_message_strategy, max_retries=st.integers(min_value=1, max_value=5))
    @settings(max_examples=50, suppress_health_check=[HealthCheck.too_slow], deadline=None)
    def test_retry_history_attached_on_exhaustion(self, message: str, max_retries: int):
    ...
    @given(message=error_message_strategy, max_retries=st.integers(min_value=1, max_value=5))
    @settings(max_examples=50, suppress_health_check=[HealthCheck.too_slow], deadline=None)
    def test_retry_history_attached_on_exhaustion_async(self, message: str, max_retries: int):
        """Retry history should be attached when async retries are exhausted."""
        policy = RetryPolicy(max_retries=max_retries, base_delay=0.001, jitter_factor=0.0)
        manager = RetryPolicyManager(policy)
    ...
    @given(message=error_message_strategy)
    @settings(max_examples=50, suppress_health_check=[HealthCheck.too_slow])
    def test_retry_history_not_attached_for_permanent_error(self, message: str):
        ...
        assert (
            "retry_history" not in error.context or len(error.context.get("retry_history", [])) == 0
        )

    @given(message=error_message_strategy, max_retries=st.integers(min_value=1, max_value=5))
    @settings(max_examples=50, suppress_health_check=[HealthCheck.too_slow], deadline=None)
    def test_retry_history_contains_error_type(self, message: str, max_retries: int):
    ...
    @given(message=error_message_strategy, max_retries=st.integers(min_value=1, max_value=3))
    @settings(max_examples=50, suppress_health_check=[HealthCheck.too_slow], deadline=None)
    def test_retry_history_delays_are_recorded(self, message: str, max_retries: int):
