...
class TestErrorStructureInvariant:
    ...
    **Validates: Requirements 1.1**
    ...
    def test_transient_error_has_required_attributes(
    ...
    @settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow])
    def test_permanent_error_has_required_attributes(
        self, message: str, context: dict[str, Any]
    ):
    ...
    @given(
        message=error_message_strategy,
        field_errors=field_errors_strategy,
        context=context_strategy
    )
    @settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow])
...
class TestCorrelationIdAutoCapture:
    ...
    **Validates: Requirements 1.10**
    ...
    @given(
        message=error_message_strategy,
        operation_name=st.text(alphabet=st.characters(whitelist_categories=('L',)), min_size=1, max_size=20)
    )
    @settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow])
    def test_transient_error_captures_correlation_id(
    ...
    @settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow])
    def test_permanent_error_captures_correlation_id(
        self, message: str, operation_name: str
    ):
    ...
    @settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow])
    def test_nested_context_uses_innermost_correlation_id(
        self, message: str, operation_name: str
    ):
...
class TestErrorSerializationRoundTrip:
    ...
    **Validates: Requirements 1.11**
    ...
    @settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow])
    def test_transient_error_serialization(
        self, message: str, context: dict[str, Any]
    ):
    ...
    @settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow])
    def test_permanent_error_serialization(
        self, message: str, context: dict[str, Any]
    ):
    ...
    @settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow])
    def test_quota_error_serialization(
        self, message: str, hours: float, context: dict[str, Any]
    ):
    ...
    @given(
        message=error_message_strategy,
        cause_message=error_message_strategy
    )
    @settings(max_examples=50, suppress_health_check=[HealthCheck.too_slow])
    def test_error_with_cause_serialization(
        self, message: str, cause_message: str
    ):
