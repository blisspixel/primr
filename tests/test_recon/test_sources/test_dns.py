...
from primr.recon.models import SourceResult
from primr.recon.sources.base import LookupSource
from primr.recon.sources.dns import DNSSource
...
class TestM365Detection:
    @pytest.mark.asyncio
    @patch("primr.recon.sources.dns._safe_resolve")
    async def test_exchange_via_mx(self, mock_resolve):
        mock_resolve.side_effect = _mock_safe_resolve_factory({
            "pepsi.com/TXT": [],
            "pepsi.com/MX": ["10 pepsi-com.mail.protection.outlook.com"],
        })
        result = await DNSSource().lookup("pepsi.com")
    ...
    async def test_exchange_via_spf(self, mock_resolve):
        mock_resolve.side_effect = _mock_safe_resolve_factory({
            "pepsi.com/TXT": ["v=spf1 include:spf.protection.outlook.com ~all"],
            "pepsi.com/MX": [],
        })
        result = await DNSSource().lookup("pepsi.com")
        assert result.m365_detected is True
    ...
    async def test_teams_via_lyncdiscover(self, mock_resolve):
        mock_resolve.side_effect = _mock_safe_resolve_factory({
            "pepsi.com/TXT": [], "pepsi.com/MX": [],
            "lyncdiscover.pepsi.com/CNAME": ["webdir.online.lync.com"],
        })
        result = await DNSSource().lookup("pepsi.com")
    ...
    async def test_intune_via_enterpriseregistration(self, mock_resolve):
        mock_resolve.side_effect = _mock_safe_resolve_factory({
            "pepsi.com/TXT": [], "pepsi.com/MX": [],
            "enterpriseregistration.pepsi.com/CNAME": ["enterpriseregistration.windows.net"],
        })
        result = await DNSSource().lookup("pepsi.com")
    ...
    async def test_dkim_exchange(self, mock_resolve):
        mock_resolve.side_effect = _mock_safe_resolve_factory({
            "pepsi.com/TXT": [], "pepsi.com/MX": [],
            "selector1._domainkey.pepsi.com/CNAME": ["sel1.protection.outlook.com"],
        })
        result = await DNSSource().lookup("pepsi.com")
    ...
    async def test_dkim_exchange_onmicrosoft(self, mock_resolve):
        ...
        mock_resolve.side_effect = _mock_safe_resolve_factory({
            "tesla.com/TXT": [], "tesla.com/MX": [],
            "selector1._domainkey.tesla.com/CNAME": ["selector1-tesla-com._domainkey.teslamotorsinc.onmicrosoft.com"],
        })
        result = await DNSSource().lookup("tesla.com")
        assert "DKIM (Exchange Online)" in result.detected_services
    ...
    async def test_autodiscover(self, mock_resolve):
        mock_resolve.side_effect = _mock_safe_resolve_factory({
            "pepsi.com/TXT": [], "pepsi.com/MX": [],
            "autodiscover.pepsi.com/CNAME": ["autodiscover.outlook.com"],
        })
        result = await DNSSource().lookup("pepsi.com")
