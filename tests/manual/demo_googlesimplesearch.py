    ...
    for start_index in range(1, max_results, 10):
        ...
        for attempt in range(1, retries + 1):
            ...
            try:
                ...
                print(Fore.RED + f"[ERROR] API request failed (Attempt {attempt}/{retries}): {e}" + Style.RESET_ALL)
                time.sleep(retry_delay)

    if structured_results:
        print(Fore.GREEN + f"[INFO] Found {len(structured_results)} valid search results." + Style.RESET_ALL)
