...
def authorize_a2a_skill(
    ...
    if is_trusted_local_a2a_context(auth_context):
        return A2ASkillAuthorizationDecision(
            allowed=True,
            skill_id=skill_id,
            required_scopes=required,
            reason="trusted_local_a2a_context",
        )
...
def a2a_scope_denied_text(
    ...
    authentication_required = decision.reason == "authentication_required"
    return json.dumps(
        {
            "error": True,
            "error_type": (
                "authentication_required" if authentication_required else "insufficient_scope"
            ),
            "error_code": (
                MCPErrorCode.AUTHENTICATION_FAILED
                if authentication_required
                else MCPErrorCode.INSUFFICIENT_SCOPE
            ),
            "message": (
                "A2A authentication is required"
                if authentication_required
                else (
