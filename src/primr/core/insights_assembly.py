...
Both functions are string assembly. File writes (``insights.txt``) stay
...
def build_combined_insights(
    ...
    *,
    ...
    ``external_text_parts`` carry verbatim scraped page bodies, and
    ``insights.txt`` flows into strategy prompts and the workbook fallback
    path - so the external block is fenced HERE (T1 boundary), making the
    persisted artifact safe everywhere it is later embedded. Downstream
    prefix slices of this string can only lose the closing marker, which
    fails safe (more text treated as data, never less).
    """
    all_insights_parts = []
    if summarized:
