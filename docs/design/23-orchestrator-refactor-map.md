...
| 6 | Cross-validation + enrichment | `_fast_cross_validate` to weak-section loop | **EXTRACTED** to `core/fast_run_validation.cross_validate_and_enrich()` (frozen `CrossValidationResult`); the coordinator injects the reviewer, while the stage owns result normalization, hostname-boundary filtering, deadline/detachment, atomic diagnostics, serial splicing, and in-place `source_urls`/`source_urls_seen` mutation; takes both `company_name` (raw, for search) and `company_label` (display) |
