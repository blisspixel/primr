...
| `PRIMR_INFERENCE_PROFILE` | Runtime capability-routing profile for wired stages. `cloud` is the default; `hybrid` enables the current routed utility-stage pilot and records body-free `stage_routes` metadata in `_run_state.json`. Prefer the `--inference` CLI flag for normal use. | `cloud` |
