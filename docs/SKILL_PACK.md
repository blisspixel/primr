...
- `BUNDLE-PATH` - bundled progressive-disclosure files use safe paths (`references/*.md`, `scripts/*.py`, the canonical `evals/evals.json`, one subdirectory, no traversal). SOFT; unsafe files are dropped at package time
...
Enabled with `--with-evals`. For each skill: generate task cases + objective expectations, run each task WITH the skill body as guidance vs WITHOUT it (baseline), grade both blind, and report the with-skill-vs-baseline pass-rate delta - proving the skill changes output. Each skill uses one case-generation call plus four fresh stateless calls per case: two task arms and one independent grader for each arm. Fresh calls retain the configured model and reasoning tier while keeping generation or with-skill history from contaminating the baseline. It also attaches `evals/evals.json` using [Anthropic's current schema](https://github.com/anthropics/skills/blob/main/skills/skill-creator/references/schemas.md), including its `expectations` field, so users can re-grade. Primr recognizes the optional upstream `files` key only when it is empty because eval input assets are not yet packaged or integrity-checked; nonempty references are rejected instead of creating dangling paths. Generated and repackaged eval fields pass one focused schema and control-plane boundary after bounded recursive canonicalization, including decoded Unicode, code-review framing, execution intent, sensitive-data access, language-aware inline comments and Python docstrings, and lexically non-public raw HTTP targets. Sixty-four decode rounds cover realistic nesting; any remaining percent or HTML entity layer fails closed. Markdown links and definitions are rejected because hidden destinations and titles are unnecessary for behavioral eval artifacts. Inert review fences may contain executable examples, but their metadata, prompt-control content, comments, and docstrings still undergo control-plane checks. Primr accepts its earlier `assertions` form when repackaging an existing bundle, but new output uses the upstream schema. The preflight estimate uses model-registry prices, bounded input and output reserves, sequential runtime, and the same strict two-attempt cap enforced by execution. OpenAI-compatible SDK retries are disabled so this explicit cap is also the HTTP-request cap. This is expensive and off by default. A request for zero cases returns without a provider call, and a skill with no admitted generated cases is skipped rather than recorded as a zero-case benchmark. A requested rerun removes any prior eval resource before generation, so rejection or provider failure cannot preserve stale evidence.
