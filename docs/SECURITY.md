...
- **T8 Stages 1-3** are shipped for MCP tool dispatch and resource reads:
  ...
  `primr://output/by_job/{job_id}` omit report bodies or previews for
  authenticated HTTP callers that do not hold `report` scope. All hide unowned jobs
  ...
  ids, granted scopes, duration, outcome, and job id when present. A2A
  ...
  `PipelineRunner` as the runtime budget. Residual risk remains around any
