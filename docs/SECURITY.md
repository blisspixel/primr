...
- **T8 Stages 1-3** are shipped for MCP tool dispatch: per-tool scopes,
  ...
  first job-scoped artifact-resource slice is also shipped:
  `primr://output/artifacts/by_job/{job_id}` returns metadata only for an owned
  job and hides unowned jobs behind the same response as missing jobs. Residual
  risk remains around A2A parity, richer job-scoped artifact resources, and any
