...
- **T8 Stages 1-3** are shipped for MCP tool dispatch: per-tool scopes,
  ...
  first two job-scoped artifact-resource slices are also shipped:
  ...
  score/status/count metadata without detailed QA body text. Both hide unowned
  jobs behind the same response as missing jobs. Residual risk remains around
  A2A parity, richer source/trace/usage resources, and any cost-incurring paths
