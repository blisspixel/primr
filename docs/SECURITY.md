...
- **T8 Stages 1-3** are shipped for MCP tool dispatch and resource reads:
  ...
  the MCP auth context, enforce `read`/`research` at skill dispatch, and own
  jobs by token `client_id`. Residual risk remains around A2A approval, audit,
  budget, and compact resource parity and any
