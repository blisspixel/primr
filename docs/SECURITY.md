...
- **T2** has complete pre-request SSRF validation and per-hop redirect
  ...
  scraper, pooled HTTPClient, and tiered requests scraper now resolve each hop
  once, validate every returned address, and connect to the validated IP literal
  while preserving original Host and HTTPS SNI. Residual SSRF risk remains in
  curl_cffi and browser-backed transport families until they receive equivalent
...
- **SSRF Protection**: All outbound URLs are validated against private IP ranges, cloud metadata endpoints, and DNS rebinding; migrated helpers also validate every redirect hop before connecting, and the shared safe HTTP and requests-family seams pin connections to the validated IP literal
