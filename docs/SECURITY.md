...
| T2 | SSRF / internal pivot | Attacker page or redirect points primr at loopback / RFC1918 / link-local / cloud-metadata | `is_safe_url` on every egress helper; per-hop redirect validation through `data/safe_http.py` for `fallback_sources._http_get`, `hiring_signals._http_get`, and Wayback CDX/replay fetches; manual per-hop validation in `data/scraping/net.py` and `HTTPClient.get/head`; final-url redirect validation on remaining legacy clients; `SSRFGuardHook` on tool calls | Shipped baseline, per-hop migration continuing |
...
- **T2** has complete pre-request SSRF validation and stronger per-hop redirect
  validation on the shared safe HTTP seam, discovery HTTP helpers, and pooled
  HTTPClient GET/HEAD calls. Some older scraping-tier clients still rely on
  final-url redirect validation and are tracked for migration to the shared seam
...
- **SSRF Protection**: All outbound URLs are validated against private IP ranges, cloud metadata endpoints, and DNS rebinding; migrated helpers also validate every redirect hop before connecting
