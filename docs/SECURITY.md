...
- **T8 Stages 1-3** are shipped for MCP tool dispatch: per-tool scopes,
  ...
  first four job-scoped artifact-resource slices are also shipped:
  ...
  citation/source appendix metadata without report body content. All hide
  unowned jobs behind the same response as missing jobs. Residual risk
  remains around A2A parity, richer source/trace resources, and any cost-incurring paths
  that do not yet have a matching estimate tool. Issue low-trust tokens with
  explicit `read` scopes rather than relying on legacy no-scope JWT defaults.
